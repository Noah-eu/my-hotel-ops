...
function normalizeNotesList(notes: string[]) {
    return notes
...
function hasMultipleNoteMentions(rawText: string) {
    const noteMentions = String(rawText || '').match(/recepce\s*:/gi) || []
    return noteMentions.length >= 2
}

function extractNameTokensFromSideText(sideText: string) {
...
function expandPrimaryGuestName(sideText: string, baseName: string | undefined) {
...
function pickDepartureGuestName(sideText: string) {
    const candidates = extractNameCandidates(String(sideText || '').split(/\r?\n/))
    return candidates[0]
}

function pickArrivalGuestName(sideText: string) {
...
function pickStayoverGuestName(rawText: string, fallbackName?: string) {
    const names: string[] = []
    const pushName = (value: string | undefined) => {
...
function inferGuestCountNearName(rawText: string, guestName?: string) {
...
function resolveArrivalGuestCountFromSide(
    roomNumber: string,
    arrivalText: string,
    currentArrivalGuestCount: number | undefined,
    departureGuestCount: number | undefined,
...
function extractStandaloneBoxNotes(notes: string[]) {
    return (notes || [])
        .map((note) => normalizeBoxText(note))
        .filter((note) => /^BOX\s+\d+$/i.test(note))
}

function extractBoxNotes(notes: string[]) {
    return (notes || [])
        .map((note) => normalizeBoxText(note))
        .filter((note) => /\bBOX\s+\d+\b/i.test(note))
}

function getSingleBoxNote(notes: string[]) {
...
export function repairSameGuestNextDayBoxContinuity(rows: PrevioStateParsedRow[]) {
    const byKey = new Map<string, PrevioStateParsedRow>()
    rows.forEach((row) => {
    ...
    rows.forEach((arrivalRow) => {
        ...
        const departureGuest = String(nextRow?.departureGuestName || '').trim()
...
export function parsePrevioStatePdfText(source: PrevioStatePdfSource, referenceDate = new Date()): PrevioStateParseResult {
    ...
    let textFallbackRows: PrevioStateParsedRow[] = []
    ...
    const nextDepartureCountByRoomAndGuest = new Map<string, number>()
